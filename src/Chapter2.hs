...
firstHalf :: [a] -> [a]
firstHalf xs = let len = length xs in take (len `div` 2) xs
...
-}
takeEven :: [a] -> [a]
takeEven xs = fst <$> (filter (\(_,b) -> even b) $ zip xs [0..])
...
smartReplicate xs = xs >>= (\x -> replicate x x)
